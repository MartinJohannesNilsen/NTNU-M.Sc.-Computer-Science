...
def calculate_accuracy(X: np.ndarray, targets: np.ndarray, model: SoftmaxModel) -> float:
    ...
    Returns:
        Accuracy (float)
    """
    # Perform forward, take argmax and count the non_zero-values giving the correct output
    # Based on assignment 1, but had to be a little bit changed because of multiple layers
    # Accuracy is correct divided by the length of output
    y_hat = np.array(model.forward(X))
    y_pred = np.argmax(y_hat, axis=1)
    y = np.argmax(targets, axis=1)
    accuracy = np.count_nonzero(y == y_pred)/X.shape[0]
...
class SoftmaxTrainer(BaseTrainer):
    ...
    def train_step(self, X_batch: np.ndarray, Y_batch: np.ndarray):
        ...
        # Perform both a forward and backward step
        model = self.model
...
if __name__ == "__main__":
    ...
    plt.ylabel("Accuracy")
    plt.legend()
    plt.savefig("../report/Images/task2c_train_loss.png")
