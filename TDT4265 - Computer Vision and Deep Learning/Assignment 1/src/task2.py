...
def calculate_accuracy(X: np.ndarray, targets: np.ndarray, model: BinaryModel) -> float:
    ...
    Returns:
        Accuracy (float)
    """
    # Perform forward, round and count the non_zero-values giving the correct output
    # Accuracy is correct divided by the length of output (targets.shape[0] works too!)

    output = model.forward(X)
    output = np.round(output, 0)
    accuracy = np.count_nonzero((output == targets))/len(output)
...
class LogisticTrainer(BaseTrainer):
    ...
    def train_step(self, X_batch: np.ndarray, Y_batch: np.ndarray):
        ...
        # Want to use numpy methods instead
        self.model.w = np.add(
            np.multiply(
                -learning_rate,
                self.model.grad),
...
if __name__ == "__main__":
    ...
    plt.savefig("img/task2b_binary_train_loss.png")
    plt.show()
    ...
    plt.ylabel("Accuracy")
    plt.legend()
    plt.savefig("img/task2c_binary_train_accuracy.png")
    plt.show()
    ...
    plt.xlabel("Number of Training Steps")
    plt.ylabel("Cross Entropy Loss - Average")
    plt.savefig("img/task2e_train_loss_with_shuffle.png")
    plt.show()
    ...
    plt.ylabel("Accuracy")
    plt.legend()
    plt.savefig("img/task2e_train_accuracy_shuffle_difference.png")
    plt.show()
