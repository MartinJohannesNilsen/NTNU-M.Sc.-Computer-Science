...
class SoftmaxModel:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO implement this function (Task 3a)
        z = X.dot(self.w)  # Outputs
        # y_hat = np.divide(np.exp(z), np.transpose(np.array([np.sum(np.exp(z), axis=1)])))
        # Neat trick
        y_hat = np.divide(np.exp(z), np.sum(np.exp(z), axis=-1, keepdims=True))
        return y_hat
...
def one_hot_encode(Y: np.ndarray, num_classes: int):
    ...
    Returns:
        Y: shape [Num examples, numerator classes]
    """
