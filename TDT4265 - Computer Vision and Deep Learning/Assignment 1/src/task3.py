...
def calculate_accuracy(X: np.ndarray, targets: np.ndarray, model: SoftmaxModel) -> float:
    ...
    y_actual_position = np.argmax(targets, axis=1)
    accuracy = np.count_nonzero(
        y_actual_position == y_predicted_position)/X.shape[0]
    return accuracy
...
if __name__ == "__main__":
    ...
    plt.savefig("img/task3b_softmax_train_loss.png")
    plt.show()
    ...
    plt.ylabel("Accuracy")
    plt.legend()
    plt.savefig("img/task3c_softmax_train_accuracy.png")
    plt.show()
