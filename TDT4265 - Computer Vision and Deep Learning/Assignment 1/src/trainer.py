...
class BaseTrainer:
    ...
    def train(
        ...
        # Setting some parameters for early stopping
        max_epochs_without_improved_loss = 10
        lowest_recorded_loss = np.inf
        ...
        for epoch in range(num_epochs):
            ...
            for X_batch, Y_batch in iter(train_loader):
                ...
                if global_step % num_steps_per_val == 0:
                    ...
                    # I am a having a discussion with myself here whether the early stopping should look at
                    # the previous loss, or the overall best loss. I started out thinking the former, but have seen
                    # other using the overall loss, and that also corresponds better to the hint of using val_history["loss"].

                    """ min_loss = min(val_history["loss"].values())
                    if min_loss < lowest_recorded_loss:
                        lowest_recorded_loss = min_loss
                        early_stop_counter = 0
                    else:
                        early_stop_counter += 1
                        if early_stop_counter == max_epochs_without_improved_loss:
