...
def pre_process_images(X: np.ndarray):
    ...
    X = (X/128)-1  # [-1,1]
    # X = X/255 # [0,1]
    X = np.append(X, bias, axis=1)  # Append the column with biases with axis=1
...
class BinaryModel:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        # Sigmoid function from Equation 1
        # An array with the same amount of rows as X and 1 column
        ...
        for i, im in enumerate(X):
            ...
            np.put(y, i, 1/(1 + np.exp(-np.dot(self.w.T, im_array))))
        return y
