...
class Map_Obj():
    ...
    def draw_map(self, map=None):
        """
        A function used to draw the map as an image.
    ...
    def append_step_visually(self, cords, gif=False, selected_path=False):
        if selected_path:
            self.set_cell_value(cords, " p ")
        else:
            self.set_cell_value(cords, " o ")
        if gif:
            self.frames.append(self.draw_map())
    ...
    def create_png(self, fname="out.png"):
        if fname.split(".")[-1] != "png":
            fname += ".png"
        image = self.draw_map()
        image.save(fname)

    def create_gif(self, fname="out.gif"):
        if fname.split(".")[-1] != "gif":
            fname += ".gif"
        self.frames[0].save(
            fname, save_all=True, append_images=self.frames[1:], duration=100, loop=0)
